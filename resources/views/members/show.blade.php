            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-end mb-3">
                        <a href="{{ route('members.anual_service_report', $member->id) }}" class="btn btn-primary">Anual
                            Service Report</a>
                    </div>
                    <ul class="list-group">
                        <li class="list-group-item"><strong>Member ID:</strong> {{ $member->member_id }}</li>
                        <li class="list-group-item"><strong>Name:</strong> {{ $member->first_name }}
                            {{ $member->last_name }}</li>
                            ...
                            @else
                                <ul>
                                    @foreach ($member->memberCourses as $memberCourse)
                                        <li class="mb-3">
                                            <div class="">
                                                {{ $memberCourse->course->name }} (Enrolled on:
                                                {{ $memberCourse->enrollment_date ? \Carbon\Carbon::parse($memberCourse->enrollment_date)->format('Y-m-d') : 'N/A' }})
                                                -
                                ...
                                <ul>
                                    @foreach ($member->memberBranches as $memberBranch)
                                        <li>
                                            ...
                                            (From:
                                            {{ $memberBranch->start_date ? \Carbon\Carbon::parse($memberBranch->start_date)->format('M d, Y') : 'N/A' }}
                                            To:
                                            {{ $memberBranch->end_date ? \Carbon\Carbon::parse($memberBranch->end_date)->format('M d, Y') : 'N/A' }})
                                            <strong>{{ $memberBranch->is_current === 'yes' ? 'Current' : '' }}</strong>
                                            <?php
                                            $branchCommittees = $member->memberCommittees
                                                ->filter(function ($mc) use ($memberBranch) {
                                                        ...
                                                        <ul>
                                                            @foreach ($roles as $role)
                                                                <li>
                                                                    ...
                                                                    (From:
                                                                    {{ $role->start_date ? \Carbon\Carbon::parse($role->start_date)->format('M d, Y') : 'N/A' }}
                                                                    To:
                                                                    {{ $role->end_date ? \Carbon\Carbon::parse($role->end_date)->format('M d, Y') : 'N/A' }})
                                                                </li>
                                                            @endforeach
                                                        </ul>
                                                    @endforeach
                        ...
                        @if ($member->blue_card_available == 'yes')
                            <li class="list-group-item">
                                ...
                                <ul>
                                    <li>Card Number : {{ $member->blue_card_number ?? 'N/A' }}</li>
                                    <li>Issue Date: {{ $member->blue_card_issue ? \Carbon\Carbon::parse($member->blue_card_issue)->format('M d, Y') : 'N/A' }}</li>
                                    <li>Expiry Date: {{ $member->blue_card_expire ? \Carbon\Carbon::parse($member->blue_card_expire)->format('M d, Y') : 'N/A' }}</li>
                                </ul>
                            </li>
                        @endif
                        @if ($member->yellow_card_available == 'yes')
                            <li class="list-group-item">
                                ...
                                <ul>
                                    <li>Card Number : {{ $member->yellow_card_number ?? 'N/A' }}</li>
                                    <li>Issue Date: {{ $member->yellow_card_issue ? \Carbon\Carbon::parse($member->yellow_card_issue)->format('M d, Y') : 'N/A' }}</li>
                                    <li>Expiry Date: {{ $member->yellow_card_expire ? \Carbon\Carbon::parse($member->yellow_card_expire)->format('M d, Y') : 'N/A' }}</li>
                                </ul>
                            </li>
                        @endif
                        <li class="list-group-item"><strong>Created By:</strong> {{ $member->creator?->name }}</li>
                        <li class="list-group-item"><strong>Updated By:</strong> {{ $member->updator?->name ?? 'N/A' }}
                        </li>
                    </ul>
